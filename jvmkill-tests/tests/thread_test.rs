...
fn thread_basic() {
    ...
    assert!(!run_java(
        "org.cloudfoundry.jvmkill.ThreadExhaustion",
        "",
        &[],
        &[
...
fn thread_time_10_count_2() {
    ...
    assert!(!run_java(
        "org.cloudfoundry.jvmkill.ThreadExhaustion",
        "=time=10,count=2,printHeapHistogram=0,printMemoryUsage=0",
        &[],
        &[
            "ResourceExhausted! (1/2)",
            "jvmkill killing current process"
        ]
    ));
}
...
fn thread_parallel_time_10_count_2() {
    let _g = THREAD_TEST_MUTEX.lock().unwrap();
    assert!(!run_java(
        "org.cloudfoundry.jvmkill.ParallelThreadExhaustion",
        "=time=10,count=2,printHeapHistogram=0,printMemoryUsage=0",
        &[],
        &["jvmkill killing current process"]
    ));
